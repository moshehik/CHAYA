...
public class ChangeBlue
{
    public static void Main(string[] args)
    {
        if (args.Length < 2)
        {
            Console.WriteLine("Usage: ChangeBlue.exe <input_image> <output_image>");
            return;
        }
        ...
        try
        {
            ...
            for (int y = 0; y < bmp.Height; y++)
            {
                for (int x = 0; x < bmp.Width; x++)
                {
                    ...
                    // Cyan / Light blue hue is generally between 170 and 250
                    if (h >= 170 && h <= 250 && s > 0.05f)
                    {
                        // Make it more vibrant
                        float newS = Math.Min(1.0f, s * 1.5f);
                        // Make it slightly darker
                        float newB = Math.Max(0.0f, b * 0.85f);
                        
                        // Shift hue slightly towards richer blue if it's very cyan
                        float newH = h;
                        if (h < 210)
                        {
                            newH = Math.Min(250f, h + 5f);
                        }
